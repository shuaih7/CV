...
def gaussian_blurred(input, ksize=9):
    blurred = cv2.GaussianBlur(img,(ksize, kszie), 0)
    return blurred
    
def average_blurred(input, ksize=15):
    blurred = cv2.blur(input, (ksize, ksize))
    return blurred
...
def create_PSF(input, ksize=15):
    img_h, img_w = input.shape[:2]
    PSF = np.zeros((img_h, img_w), dtype=np.float32)
    
    value = 1.0 / float(ksize**2)
    matrix = np.ones((ksize,ksize), dtype=np.float32) * value
    h, w = img_h//2-ksize//2, img_w//2-ksize//2
    #PSF[h:h+ksize,w:w+ksize] = matrix
    PSF[:ksize,:ksize] = matrix
    
    return PSF

if __name__ == '__main__':
    image = cv2.imread(r'C:\Users\shuai\Documents\GitHub\CV\Documents/de_ligt.jpg', -1)
    b_gray, g_gray, r_gray = cv2.split(image.copy())
    blurred_img = np.stack((average_blurred(r_gray), average_blurred(g_gray), average_blurred(b_gray)), axis=-1)
    
    PSF, eps = create_PSF(image), 150
    #reversed_img = np.stack((wiener(r_gray,PSF,eps), wiener(g_gray,PSF,eps), wiener(b_gray,PSF,eps)), axis=-1)
    reversed_img = np.stack((inverse(r_gray,PSF,eps), inverse(g_gray,PSF,eps), inverse(b_gray,PSF,eps)), axis=-1)
    plt.subplot(1,3,1), plt.imshow(cv2.cvtColor(image, cv2.COLOR_BGR2RGB)), plt.title("Original Image")
    plt.subplot(1,3,2), plt.imshow(blurred_img), plt.title("Blurred Image")
    plt.subplot(1,3,3), plt.imshow(reversed_img), plt.title("Reversed Image")
    plt.show()
